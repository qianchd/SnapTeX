...
- **Changed**: Removed repeated AST split scans, duplicate algorithm math rendering, unchanged pagination style writes, and unnecessary viewport-anchor scans.
...
- **Fixed**: Aligned AST citation metadata with rendering argument rules, including spaced optional arguments, and retained minimal AST parsing when full post-processing fails.
- **Added**: Broadened preview support for common LaTeX structures in both backends, including declared theorem and list environments, `longtable`, `minipage`, `eqnarray`, `algorithm2e`, `tikz-cd`, subfigures, and standalone graphics.
- **Added**: Rendered more reference, bibliography, and text commands, including `\cref` variants, common `siunitx` commands, and biblatex bibliography workflows.
- **Fixed**: Expanded simple `\let` aliases and macros with unbraced token arguments without leaking template internals into text or math previews.
- **Fixed**: Prevented preview-driven scrolling from triggering a delayed editor-to-preview jump, including when the editor pane is hidden.

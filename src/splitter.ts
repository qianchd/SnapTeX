    ...
    public static split(text: string): string[] {
        ...
        while ((match = regex.exec(text)) !== null) {
            ...
            if (isBegin && beginName) {
                if (!/^(proof|itemize|enumerate)$/.test(beginName)) {
                    if (/^(equation|align|gather|multline|flalign|alignat)\*?$/.test(beginName) && envStack.length === 0 && braceDepth === 0) {
                ...
                if (!/^(proof|itemize|enumerate)$/.test(endName)) {

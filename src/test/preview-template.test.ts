...
suite('Preview HTML template', () => {
    test('fills host assets and escapes attribute values', () => {
        const html = fillPreviewHtmlTemplate('<head>{{cspMeta}}\n{{styleLinks}}</head><body{{bodyData}}>{{bridgeScript}}{{scripts}}</body>', {
            ...
            bridgeScript: '<script>window.snaptexPreviewBridge=testBridge;</script>',
        ...
        assert.match(html, /window\.snaptexPreviewBridge=testBridge/);

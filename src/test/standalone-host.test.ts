...
suite('StandaloneHost', () => {
    test('switches active files while rendering from the project root', async () => {
        ...
        try {
            ...
            const update = [...messages].reverse().find((message: ExtensionToWebviewMessage) => message.command === ExtensionToWebviewCommand.Update);
            ...
            assert.ok(update && update.command === ExtensionToWebviewCommand.Update);
            assert.match(update.payload.htmls?.join('\n') ?? '', /Updated included paragraph/);
            assert.match(update.payload.htmls?.join('\n') ?? '', /Root paragraph/);
        } finally {
            restoreWindow();
        }

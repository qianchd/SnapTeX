...
SNAPTEX_RUN_USER="${SNAPTEX_RUN_USER:-$(id -un)}"
HOST="${HOST:-127.0.0.1}"
PORT="${PORT:-3000}"
...
if [[ "$SNAPTEX_INSTALL_DIR" != /* ]]; then
    echo "SNAPTEX_INSTALL_DIR must be an absolute path: $SNAPTEX_INSTALL_DIR" >&2
    exit 1
fi
SNAPTEX_INSTALL_DIR="$(realpath -m "$SNAPTEX_INSTALL_DIR")"
if [[ "$SNAPTEX_INSTALL_DIR" == "/" ]]; then
    echo "SNAPTEX_INSTALL_DIR cannot be the filesystem root." >&2
    exit 1
fi
path_contains() {
    [[ "$2" == "$1" || "$2" == "$1"/* ]]
...
if ! id "$SNAPTEX_RUN_USER" >/dev/null 2>&1; then
    echo "Service user does not exist: $SNAPTEX_RUN_USER" >&2
    exit 1
fi
...
echo "[SnapTeX] Installing dependencies and building the static Web app..."
cd "$repo_root"
npm ci
...
trap 'rm -rf "$temp_dir"; rm -f "$environment_file"' EXIT
swapped=false
had_unit=false
had_environment=false

if run_root test -f "$unit_target"; then
    run_root cp "$unit_target" "$temp_dir/previous.service"
    had_unit=true
fi
if run_root test -f "$environment_target"; then
    run_root cp "$environment_target" "$temp_dir/previous.env"
...
rollback() {
    ...
    if run_root test -d "$backup_dir"; then
        run_root mv "$backup_dir" "$SNAPTEX_INSTALL_DIR"
    fi
    if [[ "$had_unit" == true ]]; then
        run_root install -m 0644 "$temp_dir/previous.service" "$unit_target"
    else
        run_root rm -f "$unit_target"
    fi
    if [[ "$had_environment" == true ]]; then
        run_root install -m 0600 "$temp_dir/previous.env" "$environment_target"
    else
        run_root rm -f "$environment_target"
    fi
...
run_root install -o "$SNAPTEX_RUN_USER" -g "$run_group" -m 0644 \
    "$repo_root/apps/web/server.mjs" "$next_dir/apps/web/server.mjs"
run_root chown -R "$SNAPTEX_RUN_USER:$run_group" "$next_dir"

if run_root test -d "$SNAPTEX_INSTALL_DIR"; then
    run_root mv "$SNAPTEX_INSTALL_DIR" "$backup_dir"
    swapped=true
fi
...
health_url="http://$health_host:$PORT/api/projects"
for _ in {1..20}; do
